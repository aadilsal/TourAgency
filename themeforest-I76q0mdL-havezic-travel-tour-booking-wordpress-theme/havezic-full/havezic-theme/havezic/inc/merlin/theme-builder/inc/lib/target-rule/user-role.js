;(function ( $, window, undefined ) {
	...
	var user_role_update_close_button = function(wrapper) {
		...
		type 		= wrapper.closest('.etb-user-role-wrapper').attr('data-type');
		...
		rules.each(function() {
			if ( show_close ) {
				jQuery(this).find('.user_role-condition-delete').removeClass('etb-hidden');
			}else{
				jQuery(this).find('.user_role-condition-delete').addClass('etb-hidden');
			}
		});
	};
	...
	$(document).ready(function($) {
		...
		jQuery('.etb-user-role-selector-wrapper').each(function() {
			user_role_update_close_button( jQuery(this) );
		})
		
		jQuery( '.etb-user-role-selector-wrapper' ).on( 'click', '.user_role-add-rule-wrap a', function(e) {
			e.preventDefault();
			e.stopPropagation();
			var $this 		= jQuery( this ),
				id 			= $this.attr( 'data-rule-id' ),
				new_id 		= parseInt(id) + 1,
				rule_wrap 	= $this.closest('.etb-user-role-selector-wrapper').find('.user_role-builder-wrap'),
				template  	= wp.template( 'astra-user-role-condition' ),
				field_wrap 	= $this.closest('.etb-user-role-wrapper');
		...
		jQuery( '.etb-user-role-selector-wrapper' ).on( 'click', '.user_role-condition-delete', function(e) {
			var $this 			= jQuery( this ),
			...
			field_wrap.find('.astra-user-role-condition').each(function(i) {
				...
				condition.removeClass('etb-user-role-'+old_rule_id).addClass('etb-user-role-'+i);
