...
namespace Theme_Builder\Lib;

/**
 * Meta Boxes setup
 */
class ETB_Target_Rules_Fields {
    ...
    public function __construct() {
        define('ASTRA_TARGET_RULE_URI', OPAL_BUILDER_URL . 'inc/lib/target-rule/');
        define('ASTRA_TARGET_RULE_VER', '1.0.0');
        define('ASTRA_TARGET_RULE_DIR', trailingslashit( dirname( __FILE__ ) ));
        add_action('admin_action_edit', array($this, 'initialize_options'));
        add_action('wp_ajax_etb_get_posts_by_query', array($this, 'etb_get_posts_by_query'));
    }
    ...
    public static function get_location_selections() {
        ...
        $special_pages = array(
            'special-404'    => __('404 Page', 'havezic'),
            'special-search' => __('Search Page', 'havezic'),
            'special-blog'   => __('Blog / Posts Page', 'havezic'),
            'special-front'  => __('Front Page', 'havezic'),
            'special-date'   => __('Date Archive', 'havezic'),
            'special-author' => __('Author Archive', 'havezic'),
        );

        if (class_exists('WooCommerce')) {
            $special_pages['special-woo-shop'] = __('WooCommerce Shop Page', 'havezic');
        }

        $selection_options = array(
            'basic' => array(
                'label' => __('Basic', 'havezic'),
                'value' => array(
                    'basic-global'    => __('Entire Website', 'havezic'),
                    'basic-singulars' => __('All Singulars', 'havezic'),
                    'basic-archives'  => __('All Archives', 'havezic'),
                ),
            ),

            'special-pages' => array(
                'label' => __('Special Pages', 'havezic'),
                'value' => $special_pages,
            ),
        );
        ...
        $selection_options['specific-target'] = array(
            'label' => __('Specific Target', 'havezic'),
            'value' => array(
                'specifics' => __('Specific Pages / Posts / Taxonomies, etc.', 'havezic'),
            ),
        );
    ...
    public static function get_user_selections() {
        $selection_options = array(
            'basic' => array(
                ...
                'value' => array(
                    'all'        => __('All', 'havezic'),
                    'logged-in'  => __('Logged In', 'havezic'),
                    'logged-out' => __('Logged Out', 'havezic'),
                ),
            ),

            'advanced' => array(
                'label' => __('Advanced', 'havezic'),
                'value' => array(),
            ),
        );
     ...
     * @since  1.0.0
     */
    function etb_get_posts_by_query() {

        check_ajax_referer('theme-builder-get-posts-by-query', 'nonce');
        ...
        unset($post_types['etb_library']); //Exclude templates.
     ...
     * This is only run from etb_get_posts_by_query()
     *
    ...
    public function admin_styles() {
        wp_enqueue_script('astra-select2', ASTRA_TARGET_RULE_URI . 'select2.js', array('jquery'), ASTRA_TARGET_RULE_VER, true);
        ...
        if ('' !== $wp_lang) {
            ...
            if (isset($select2_lang[$wp_lang]) && file_exists(ASTRA_TARGET_RULE_DIR . 'i18n/' . $select2_lang[$wp_lang] . '.js')) {
                $ast_lang = $select2_lang[$wp_lang];
                wp_enqueue_script(
                    'astra-select2-lang',
                    ASTRA_TARGET_RULE_URI . 'i18n/' . $select2_lang[$wp_lang] . '.js',
                    array(
                        'jquery',
                        'astra-select2',
                    ),
                    ASTRA_TARGET_RULE_VER,
                    true
                );
            }
        }
        ...
        wp_register_script(
            'astra-target-rule',
            ASTRA_TARGET_RULE_URI . 'target-rule.js',
            array(
                'jquery',
                'astra-select2',
            ),
        ...
        wp_register_script(
            'astra-user-role',
            ASTRA_TARGET_RULE_URI . 'user-role.js',
            array(
                'jquery',
            ),
            ASTRA_TARGET_RULE_VER,
        ...
        wp_register_style('astra-select2', ASTRA_TARGET_RULE_URI . 'select2.css', '', ASTRA_TARGET_RULE_VER);
        wp_enqueue_style('astra-select2');
        wp_register_style('astra-target-rule', ASTRA_TARGET_RULE_URI . 'target-rule.css', '', ASTRA_TARGET_RULE_VER);
        ...
        $localize_vars = array(
            'ast_lang'      => $ast_lang,
            'please_enter'  => __('Please enter', 'havezic'),
            'please_delete' => __('Please delete', 'havezic'),
            'more_char'     => __('or more characters', 'havezic'),
            'character'     => __('character', 'havezic'),
            'loading'       => __('Loading more results…', 'havezic'),
            'only_select'   => __('You can only select', 'havezic'),
            'item'          => __('item', 'havezic'),
            'char_s'        => __('s', 'havezic'),
            'no_result'     => __('No results found', 'havezic'),
            'searching'     => __('Searching…', 'havezic'),
            'not_loader'    => __('The results could not be loaded.', 'havezic'),
            'search'        => __('Search pages / post / categories', 'havezic'),
            'ajax_nonce'    => wp_create_nonce('theme-builder-get-posts-by-query'),
        );
    ...
    public static function target_rule_settings_field($name, $settings, $value) {
        ...
        $add_rule_label = isset($settings['add_rule_label']) ? $settings['add_rule_label'] : __('Add Rule', 'havezic');
        $saved_values   = $value;
        $output         = '';
        ...
        $output .= '<div class="astra-target-rule-condition etb-target-rule-{{data.id}}" data-rule="{{data.id}}" >';
        ...
        $output .= '<select name="' . esc_attr($input_name) . '[rule][{{data.id}}]" class="target_rule-condition form-control etb-input">';
        $output .= '<option value="">' . __('Select', 'havezic') . '</option>';
        ...
        $output .= '<select name="' . esc_attr($input_name) . '[specific][]" class="target-rule-select2 target_rule-specific-page form-control etb-input " multiple="multiple">';
        $output .= '</select>';
        $output .= '</div>';
        ...
        /* Wrapper Start */
        $output .= '<div class="etb-target-rule-wrapper etb-target-rule-' . $rule_type . '-on-wrap" data-type="' . $rule_type . '">';
        // $output .= '<input type="hidden" class="form-control etb-input etb-target_rule-input" name="' . esc_attr( $input_name ) . '" value=' . $value . ' />';
        $output .= '<div class="etb-target-rule-selector-wrapper etb-target-rule-' . $rule_type . '-on">';
    ...
    public static function get_post_target_rule_options($post_type, $taxonomy) {
        ...
        $all_posts                        = sprintf(__('All %s', 'havezic'), $post_label);
        $post_option[$post_name . '|all'] = $all_posts;

        if ('pages' != $post_key) {
            /* translators: %s post label */
            $all_archive                              = sprintf(__('All %s Archive', 'havezic'), $post_label);
        ...
        if (in_array($post_type->name, $taxonomy->object_type)) {
            ...
            $tax_archive = sprintf(__('All %s Archive', 'havezic'), $tax_label);
    ...
    public static function generate_target_rule_selector($type, $selection_options, $input_name, $saved_values, $add_rule_label) {
        ...
        if (is_array($saved_values) && is_array($saved_values['rule'])) {
            foreach ($saved_values['rule'] as $index => $data) {
                $output .= '<div class="astra-target-rule-condition etb-target-rule-' . $index . '" data-rule="' . $index . '" >';
                /* Condition Selection */
                $output .= '<span class="target_rule-condition-delete dashicons dashicons-dismiss"></span>';
                $output .= '<div class="target_rule-condition-wrap" >';
                $output .= '<select name="' . esc_attr($input_name) . '[rule][' . $index . ']" class="target_rule-condition form-control etb-input">';
                $output .= '<option value="">' . __('Select', 'havezic') . '</option>';
        ...
        if ('display' == $type) {
            ...
            $output .= '<a href="#" class="button">' . __('Add Exclusion Rule', 'havezic') . '</a>';
            $output .= '</div>';
        }
                            ...
                            if (is_archive()) {
                                ...
                                if ($current_post_type == $post_type) {
                                    if ('archive' == $archieve_type) {
                                        ...
                                        if ($obj && is_object($obj) && property_exists($obj, 'taxonomy')) {
    ...
    public static function target_user_role_settings_field($name, $settings, $value) {
        ...
        $output .= '<div class="astra-user-role-condition etb-user-role-{{data.id}}" data-rule="{{data.id}}" >';
        $output .= '<span class="user_role-condition-delete dashicons dashicons-dismiss"></span>';
        /* Condition Selection */
        $output .= '<div class="user_role-condition-wrap" >';
        $output .= '<select name="' . esc_attr($input_name) . '[{{data.id}}]" class="user_role-condition form-control etb-input">';
        $output .= '<option value="">' . __('Select', 'havezic') . '</option>';
        ...
        $output .= '<div class="etb-user-role-wrapper etb-user-role-display-on-wrap" data-type="display">';
        $output .= '<div class="etb-user-role-selector-wrapper etb-user-role-display-on">';
        $output .= '<div class="user_role-builder-wrap">';
        foreach ($saved_values as $index => $data) {
            $output .= '<div class="astra-user-role-condition etb-user-role-' . $index . '" data-rule="' . $index . '" >';
            $output .= '<span class="user_role-condition-delete dashicons dashicons-dismiss"></span>';
            /* Condition Selection */
            $output .= '<div class="user_role-condition-wrap" >';
            $output .= '<select name="' . esc_attr($input_name) . '[' . $index . ']" class="user_role-condition form-control etb-input">';
            $output .= '<option value="">' . __('Select', 'havezic') . '</option>';
        ...
        if (!empty($already_set_rule)) {
            add_action(
                ...
                function () use ($already_set_rule) {
                    ...
                    $notice = sprintf(__('The same display setting is already exist in %s post/s.', 'havezic'), $rule_set_titles);
 ...
 */
ETB_Target_Rules_Fields::get_instance();
