<?php
/**
 * Theme_Builder_Elementor_Canvas_Compat setup
 *
 ...
 */
class Theme_Builder_Elementor_Canvas_Compat {

	/**
	 * Instance of Theme_Builder_Elementor_Canvas_Compat.
	 *
	 * @var Theme_Builder_Elementor_Canvas_Compat
	 */
	...
	public static function instance() {
		if ( ! isset( self::$instance ) ) {
			self::$instance = new Theme_Builder_Elementor_Canvas_Compat();
	...
	public function hooks() {
        ...
        if ( etb_after_header_enabled() ) {
            ...
            if ( 'elementor_canvas' == get_page_template_slug() ) {
                $override_cannvas_template = get_post_meta( etb_get_before_footer_id(), 'display-on-canvas-template', true );

                if ( '1' == $override_cannvas_template ) {
                    add_action( 'elementor/page_templates/canvas/after_content', 'etb_render_after_header', 9 );
                }
            }
        }
		...
		if ( etb_is_before_footer_enabled() ) {
			...
			if ( 'elementor_canvas' == get_page_template_slug() ) {
				...
				if ( '1' == $override_cannvas_template ) {
					add_action( 'elementor/page_templates/canvas/after_content', 'etb_render_before_footer', 9 );
				}
			}
		}
	}
	...
	public function render_header() {
		...
		$override_cannvas_template = get_post_meta( get_etb_header_id(), 'display-on-canvas-template', true );
	...
	public function render_footer() {
		...
		$override_cannvas_template = get_post_meta( get_etb_footer_id(), 'display-on-canvas-template', true );
...
Theme_Builder_Elementor_Canvas_Compat::instance();
