;(function ( $, window, undefined ) {
	...
	var update_target_rule_input = function(wrapper) {
		var rule_input 		= wrapper.find('.etb-target_rule-input');
	...
	var update_close_button = function(wrapper) {
		...
		type 		= wrapper.closest('.etb-target-rule-wrapper').attr('data-type');
		...
		rules.each(function() {
			if ( show_close ) {
				jQuery(this).find('.target_rule-condition-delete').removeClass('etb-hidden');
			}else{
				jQuery(this).find('.target_rule-condition-delete').addClass('etb-hidden');
			}
		});
	};

	var update_exclusion_button = function( force_show, force_hide ) {
		var display_on = $('.etb-target-rule-display-on-wrap');
		var exclude_on = $('.etb-target-rule-exclude-on-wrap');
		...
		if ( true == force_hide ) {
			exclude_field_wrap.addClass( 'etb-hidden' );
			add_exclude_block.removeClass( 'etb-hidden' );
		}else if( true == force_show ){
			exclude_field_wrap.removeClass( 'etb-hidden' );
			add_exclude_block.addClass( 'etb-hidden' );
		}else{
	...
	$(document).ready(function($) {
		...
		jQuery('.etb-target-rule-selector-wrapper').each(function() {
		...
		jQuery( document ).on( 'change', '.astra-target-rule-condition select.target_rule-condition' , function( e ) {
			...
			var $this 		= jQuery(this),
				this_val 	= $this.val(),
				field_wrap 	= $this.closest('.etb-target-rule-wrapper');
		...
		jQuery( '.etb-target-rule-selector-wrapper' ).on( 'change', '.target-rule-select2', function(e) {
			var $this 		= jQuery( this ),
				field_wrap 	= $this.closest('.etb-target-rule-wrapper');
		...
		jQuery( '.etb-target-rule-selector-wrapper' ).on( 'click', '.target_rule-add-rule-wrap a', function(e) {
			e.preventDefault();
			e.stopPropagation();
			var $this 	= jQuery( this ),
				...
				rule_wrap = $this.closest('.etb-target-rule-selector-wrapper').find('.target_rule-builder-wrap'),
				template  = wp.template( 'astra-target-rule-' + type + '-condition' ),
				field_wrap 		= $this.closest('.etb-target-rule-wrapper');

			rule_wrap.append( template( { id : new_id, type : type } ) );
			
			init_target_rule_select2( '.etb-target-rule-'+type+'-on .target-rule-select2' );
		...
		jQuery( '.etb-target-rule-selector-wrapper' ).on( 'click', '.target_rule-condition-delete', function(e) {
			var $this 			= jQuery( this ),
			...
			field_wrap.find('.astra-target-rule-condition').each(function(i) {
				...
				condition.removeClass('etb-target-rule-'+old_rule_id).addClass('etb-target-rule-'+i);
		...
		jQuery( '.etb-target-rule-selector-wrapper' ).on( 'click', '.target_rule-add-exclusion-rule a', function(e) {
			e.preventDefault();
			e.stopPropagation();
