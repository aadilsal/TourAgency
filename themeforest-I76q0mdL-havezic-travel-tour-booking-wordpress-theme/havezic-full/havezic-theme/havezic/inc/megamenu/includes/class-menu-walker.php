 ...
 */
class Havezic_Megamenu_Walker extends Walker_Nav_Menu {
    private $custom_param;
    ...
    public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
        ...
        if (isset($item->mega_data) && !empty($item->mega_data)) {
            if ($this->custom_param && $this->custom_param['show_icon']) {
                if (!empty($item->mega_data['icon'])) {
                    $icon_class = $item->mega_data['icon'];
                    if ($icon_class == 1) {
                        $icon_class = $item->mega_data['icon-custom'];
                    }
                    $icon = havezic_megamenu_get_icon_html($item->mega_data, $icon_class);
                }
            }
            if (!empty($item->mega_data['badge_title'])) {
                $badge = havezic_get_badge_html($item->mega_data['badge_title'], $item->mega_data);
            }
        ...
        $title = sprintf(
            '%1$s<span class="menu-title" %2$s>%3$s</span>%4$s',
            $icon,
            $color,
            $title,
            $badge
    ...
    public function display_element($element, &$children_elements, $max_depth, $depth, $args, &$output) {
        ...
        $element->mega_data = havezic_megamenu_get_item_data($element->ID);
    ...
    public function is_megamenu($item, $depth) {
        if ($this->custom_param && $this->custom_param['is_megamenu']) {
            if ($depth == 0) {
    ...
    public function render_megamenu_elementor($item, $args, $depth) {
        ...
        if (class_exists('Elementor\Plugin')) {
            $post_id = havezic_megamenu_get_post_related_menu($item->ID);
