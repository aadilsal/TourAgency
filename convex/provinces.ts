...
/**
 * Adds any default provinces whose slug is missing. INSERT-ONLY: existing rows
 * are never modified, so admin edits can't be overwritten by the seed.
 * (`updated` is kept in the return shape for older clients and is always 0.)
 */
export const syncDefaultCatalog = mutation({
  ...
  handler: async (ctx, { sessionToken }) => {
    ...
    for (let i = 0; i < DEFAULT_PROVINCES.length; i++) {
      ...
      await ctx.db.insert("provinces", { ...row, createdAt: now + i, updatedAt: now + i });
      created++;
    }
    return { created, updated: 0, skipped, total: DEFAULT_PROVINCES.length };
  },
});
...
export const listForIndex = query({
  args: {},
  handler: async (ctx) => {
    const provinces = (await ctx.db.query("provinces").collect()).filter(
      (p) => !p.autoGenerated,
    );
...
export const listSlugs = query({
  ...
  handler: async (ctx) => {
    ...
    return rows.filter((p) => !p.autoGenerated).map((p) => p.slug);
  },
});
    ...
    return Promise.all(
      rows
        ...
        .map(async (p) => ({
          ...
          destinationCount: destinations.filter((d) => d.provinceSlug === p.slug).length,
        })),
    );
  },
});
...
export const updateProvince = mutation({
  args: {
    ...
    /** false = show on public /guides (publish an AI-created province). */
  ...
  handler: async (ctx, { sessionToken, provinceId, ...patch }) => {
    ...
    if (patch.name !== undefined && !patch.name.trim()) {
      throw new Error("Province name can't be empty");
    }
...
/** What deleting a province would affect — shown to the admin before confirming. */
export const getDeletionImpact = query({
  ...
  handler: async (ctx, { sessionToken, provinceId }) => {
    await requireAdminFromSession(ctx, sessionToken);
    const province = await ctx.db.get(provinceId);
    if (!province) return null;
    const sites = await ctx.db
      .query("sites")
      .withIndex("by_province", (q) => q.eq("provinceId", provinceId))
      .collect();
    const destinations = (await ctx.db.query("destinations").collect()).filter(
      (d) => d.provinceSlug === province.slug,
    );
    return {
      siteCount: sites.length,
      destinationCount: destinations.length,
      destinationNames: destinations.slice(0, 10).map((d) => d.name),
    };
  },
});
...
export const deleteProvince = mutation({
  ...
  handler: async (ctx, { sessionToken, provinceId }) => {
    ...
    const province = await ctx.db.get(provinceId);
    if (!province) return;
    // Never cascade-delete curated site content: the admin must move or delete it first.
    ...
    if (siteRows.length > 0) {
      throw new Error(
        `"${province.name}" still has ${siteRows.length} site(s). Move or delete its sites first, then delete the province.`,
      );
    }
    const tours = await ctx.db.query("tours").collect();
    for (const t of tours) {
      if (Array.isArray(t.provinceIds) && t.provinceIds.includes(provinceId)) {
        await ctx.db.patch(t._id, {
          ...
          // Bump the revision so an open tour editor detects the change
          // instead of silently re-saving the removed province.
          updatedAt: Math.max(Date.now(), (t.updatedAt ?? t.createdAt) + 1),
        });
      }
    }
