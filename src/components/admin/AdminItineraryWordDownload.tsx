...
import { alignHotelsToRows, tiersToPackagesForPdf, type PackageTier } from "@/lib/itineraryPackageMatrix";
...
export function AdminItineraryWordDownload({ itineraryId }: { itineraryId: string }) {
  ...
  /** True until uploaded images have URLs — rendering earlier drops them from the file. */
  ...
  const pdfModel = useMemo(() => {
    ...
    return {
      layoutVariant: "advanced" as const,
      headline: itin.headline,
      variantLabel: itin.variantLabel,
      tripTitle: itin.title,
      coverSubtitle: itin.coverSubtitle || undefined,
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const blob = await buildItineraryWordBlob(pdfModel);
        const url = URL.createObjectURL(blob);
        const a = document.createElement("a");
        a.href = url;
        a.download = `${(itin.title || "itinerary").replace(/\s+/g, "-").toLowerCase()}.docx`;
  ...
  return (
    <div className="space-y-3">
      <h1 className="text-xl font-semibold text-foreground">Downloading Word file…</h1>
      <p className="text-sm text-muted">{busy ? "Preparing file…" : "If your download didn’t start, use the link below."}</p>
